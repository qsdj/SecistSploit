...
from protocol.http.HTTPClient import HTTPClient
...
class Exploit(HTTPClient):
    ...
    target = StringOption("", "Target URl")
    port = PortOption(80, "Target HTTP port")
    ...
    def run(self):
        ...
        for r in dk.readlines():
            sc = "".join(r.split('\n'))
            response = self.http_request(method="GET", path="{}".format(sc))
            for b in blackhit:
